...
//  std::cout<<"using weights!!"<<std::endl;
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
      ...
      Dtype w = static_cast<Dtype>(weight[i * inner_num_ + j]);
/*		
	  if (w < 1)
		w = 0.5;
...
*/
/*
	  if (label_value == 1 && w != 255) {
		std::cout<<j<<":"<<(int)w<<" ";
	 }
*/
//      Dtype w = (label_value == 1) ? 10:1; 
//      std::cout<<w<<std::endl;
//	  Dtype w = weight_data[j];
//	  if (w!=Dtype(1) && i==0)
//		std::cout<<"weight is not one "<<w<<std::endl;
  ...
  if (propagate_down[0]) {
   ...
   if( bottom.size() == 3) {
    ...
    //      Dtype w = weight[i * inner_num_ + j];
   	//	  int w = static_cast<int>(weight[i * inner_num_ + j]);
