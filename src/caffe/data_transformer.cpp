...
void DataTransformer<Dtype>::Transform(Blob<Dtype>* input_blob,
                                       Blob<Dtype>* transformed_blob, bool is_img) {
  const int crop_size = param_.crop_size();
...
template<typename Dtype>
void DataTransformer<Dtype>::TransformImgAndSegAndWeight(
    const std::vector<cv::Mat>& cv_img_seg,
    Blob<Dtype>* transformed_data_blob,
    Blob<Dtype>* transformed_label_blob,
    Blob<Dtype>* transformed_weight_blob,
    const int ignore_label) {
  CHECK(cv_img_seg.size() == 3) << "Input must contain image and seg and weight.";
  ...
  const int weight_channels = cv_img_seg[2].channels();
  int weight_height   = cv_img_seg[2].rows;
  int weight_width    = cv_img_seg[2].cols;
  ...
  const int t_weight_channels = transformed_weight_blob->channels();
  const int t_weight_height   = transformed_weight_blob->height();
  const int t_weight_width    = transformed_weight_blob->width();

  CHECK_EQ(seg_channels, 1);
  CHECK_EQ(img_channels, data_channels);
  CHECK_EQ(img_height, seg_height);
  CHECK_EQ(img_width, seg_width);
  CHECK_EQ(weight_channels, 1);
  CHECK_EQ(img_height, weight_height);
  CHECK_EQ(img_width, weight_width);

  CHECK_EQ(label_channels, 1);
  CHECK_EQ(data_height, label_height);
  CHECK_EQ(data_width, label_width);
  CHECK_EQ(t_weight_channels, 1);
  CHECK_EQ(data_height, t_weight_height);
  CHECK_EQ(data_width, t_weight_width);
  ...
  CHECK(cv_img_seg[1].depth() == CV_8U)
      << "Seg data type must be unsigned byte";
  CHECK(cv_img_seg[2].depth() == CV_32F)
      << "Weight data type must be float";
  ...
  if (scale_factors_.size() > 0) {
    ...
    if (scale != 1) {
      ...
      cv::resize(cv_img_seg[1], cv_cropped_seg, cv::Size(img_width, img_height), 0, 0, 
		 cv::INTER_NEAREST);
      cv::resize(cv_img_seg[2], cv_cropped_weight, cv::Size(img_width, img_height), 0, 0, 
		 cv::INTER_LINEAR);
    } else {
      cv_cropped_img = cv_img_seg[0];
      cv_cropped_seg = cv_img_seg[1];
      cv_cropped_weight = cv_img_seg[2];
    }
  } else {
    cv_cropped_img = cv_img_seg[0];
    cv_cropped_seg = cv_img_seg[1];
...
/*
	std::cout<<"Dt char"<<std::endl;
	for (int i=0; i<cv_cropped_seg.rows; i++)
		for (int j=0; j<cv_cropped_seg.cols; j++) {
			if ((int)cv_cropped_seg.at<uchar>(i,j) == 1 && (int)cv_cropped_weight.at<uchar>(i,j) != 255) {
				std::cout<<i<<","<<j<<":"<<(int)cv_cropped_weight.at<uchar>(i,j)<<" ";			
}
}
	std::cout<<std::endl;
*/
...
//  cv_cropped_weight.convertTo(cv_cropped_weight, CV_32F);
...
//          cv::Scalar(0));
    cv::copyMakeBorder(cv_cropped_seg, cv_cropped_seg, 0, pad_height,
          0, pad_width, cv::BORDER_CONSTANT,
          cv::Scalar(ignore_label));
    cv::copyMakeBorder(cv_cropped_weight, cv_cropped_weight, 0, pad_height,
          0, pad_width, cv::BORDER_CONSTANT,
          cv::Scalar(0));
    // update height/width
    img_height   = cv_cropped_img.rows;
    ...
    weight_height   = cv_cropped_weight.rows;
    weight_width    = cv_cropped_weight.cols;

  }

/*
	std::cout<<"Dt pad"<<std::endl;
	for (int i=0; i<cv_cropped_seg.rows; i++)
		for (int j=0; j<cv_cropped_seg.cols; j++) {
			if ((int)cv_cropped_seg.at<uchar>(i,j) == 1 && (int)cv_cropped_weight.at<uchar>(i,j) != 255) {
				std::cout<<i<<","<<j<<":"<<(int)cv_cropped_weight.at<uchar>(i,j)<<" ";			
...
*/
  ...
  if (crop_width && crop_height) {
    ...
    cv_cropped_img = cv_cropped_img(roi);
    cv_cropped_seg = cv_cropped_seg(roi);
    cv_cropped_weight = cv_cropped_weight(roi);
  }

/*
	std::cout<<"Dt crop"<<std::endl;
	for (int i=0; i<cv_cropped_seg.rows; i++)
		for (int j=0; j<cv_cropped_seg.cols; j++) {
			if ((int)cv_cropped_seg.at<uchar>(i,j) == 1 && (int)cv_cropped_weight.at<uchar>(i,j) != 255) {
				std::cout<<i<<","<<j<<":"<<(int)cv_cropped_weight.at<uchar>(i,j)<<" ";			
...
*/
  ...
  CHECK(cv_cropped_img.data);
  CHECK(cv_cropped_seg.data);
  CHECK(cv_cropped_weight.data);

  Dtype* transformed_data  = transformed_data_blob->mutable_cpu_data();
  Dtype* transformed_label = transformed_label_blob->mutable_cpu_data();
  Dtype* transformed_weight = transformed_weight_blob->mutable_cpu_data();

//	std::cout<<"Dt transform"<<std::endl;
  ...
  for (int h = 0; h < data_height; ++h) {
    data_ptr = cv_cropped_img.ptr<double>(h);
    label_ptr = cv_cropped_seg.ptr<uchar>(h);
    weight_ptr = cv_cropped_weight.ptr<float>(h);
    ...
    for (int w = 0; w < data_width; ++w) {
      ...
      Dtype weight = static_cast<Dtype>(weight_ptr[weight_index++]);
      transformed_weight[top_index] = weight;


//	  if ((int)pixel == 1 && (int)weight != 255)
//		std::cout<<top_index<<"->"<<weight<<" ";
