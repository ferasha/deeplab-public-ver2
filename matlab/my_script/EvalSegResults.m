...
%    VOC_root_folder = '/rmt/data/pascal/VOCdevkit';
    VOC_root_folder = '/home/rasha/Downloads/VOCdevkit';
  elseif strcmp(dataset, 'coco')
    VOC_root_folder = '/rmt/data/coco';
  elseif strcmp(dataset, 'drusen')
    VOC_root_folder = '/home/rasha/OCT-Data/ground-truth-with-drusens/test';
  elseif strcmp(dataset, 'drusen_area')
%    VOC_root_folder = '/home/rasha/OCT-Data/ground-truth-with-drusens/test';
%    VOC_root_folder = '/home/rasha/Desktop/test_area';
    VOC_root_folder = '/home/rasha/Desktop/DataFromUniClinic/Input-With-Label';
  else
    error('Wrong dataset');
  end
else
...
%output_mat_folder = fullfile('/home/rasha/work/deeplab-public-ver2', dataset, feature_name, model_name, testset, feature_type);

output_mat_folder = fullfile('/home/rasha/work/deeplab-public-ver2', dataset, feature_name, model_name, testset, 'small', 'snapshot_');

save_root_folder = fullfile('/home/rasha/work/deeplab-public-ver2', dataset, feature_name, model_name, testset, feature_type, post_folder);
...
elseif strcmp(dataset, 'drusen')
  seg_res_dir = [save_root_folder '/results/drusen/'];
  seg_root = fullfile(VOC_root_folder, '');
  gt_dir   = fullfile(VOC_root_folder, '');
elseif strcmp(dataset, 'drusen_area')
  seg_res_dir = [save_root_folder '/results/drusen_area/'];
  seg_root = fullfile(VOC_root_folder, '');
  gt_dir   = fullfile(VOC_root_folder, '');
end
...
elseif strcmp(dataset, 'drusen')
  VOCopts = GetVOCopts(seg_root, seg_res_dir, trainset, testset, 'drusen');
elseif strcmp(dataset, 'drusen_area')
  VOCopts = GetVOCopts(seg_root, seg_res_dir, trainset, testset, 'drusen_area');
end

if is_mat
  % crop the results
%  load('pascal_seg_colormap.mat');
  load('drusen_seg_colormap.mat');
  ...
  for i = 1 : numel(output_dir)
    ...
    elseif strcmp(dataset, 'drusen')
      img = imread(fullfile(VOC_root_folder, [img_fn, '.tif']));
    elseif strcmp(dataset, 'drusen_area')
 %     img = imread(fullfile(VOC_root_folder, [img_fn, '.tif']));
       img = imread(fullfile(VOC_root_folder, path_split{1}, path_split{2}, path_split{3}, [path_split{4}, '-Input.tif']));
    end
...
%    imwrite(result, colormap, fullfile(save_result_folder, [img_fn, '.png']));
	...
	save_result_folder = output_mat_folder;
	pred_folder = fullfile(save_result_folder, 'matlab_images', path_split{1}, path_split{2}, path_split{3});
	if ~exist(pred_folder, 'dir')
	    mkdir(pred_folder);
    end
    imwrite(result, colormap, fullfile(pred_folder, [path_split{4}, '-pred.png']));
    base = 0;
    for th = 1 : 11
        fprintf(1, 'base %d...\n', base);
        sec_c = uint8(raw_result(:,:,2) > base);
 %       imwrite(sec_c, colormap, fullfile(pred_folder, [path_split{4}, '-', int2str(th), '_no_interp-pred.png']));
        imwrite(sec_c, colormap, fullfile(pred_folder, [path_split{4}, '-', int2str(th), '-pred.png']));
        base = base + 0.1;
    end
  end
end
